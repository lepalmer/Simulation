...
def plotAeffvsEnergy(energy, aeff, aeff_eres, aeff_eres_modfrac,
    ...
    plt.title(r'Effective Area vs. Energy ($\zenith$ = {:,.0f}$^\circ$, ' +
              '$\azimuth$ = {:,.0f}$^\circ$)'.format(ze, az))
...
def plotAeff(simFiles, axis='ze', plotGBM=False):
    ...
    axis : string
       The axis to plot against (either `ze` or `az`)
    ...
    for angle in set(aeffs[axis]):
        mask = aeffs[axis] == angle
    ...
    for energy in set(aeffs['keV']):
        ...
        plotAeffvsTheta(aeffs[axis][mask],
                        aeffs['aeff'][mask],
                        aeffs['aeff_eres'][mask],
                        aeffs['aeff_eres_modfrac'][mask],
                        energy)
